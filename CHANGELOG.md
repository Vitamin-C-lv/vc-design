...
### 2026-09-20 · 四处图注 / alt 与图不符（图片内容核查找出来的）

**为什么** — 换完第 04 章那两幅图之后，派了两个 agent 做**图片内容核查**（逐张把图打开，跟 `projects.ts` 里的 caption / alt 并排看），共看 68 张，报出 4 处不符。我逐条打开原图复核 —— **四条全部成立，而且四条都是文案写错了，图本身没问题**：

| key | 文案说 | 图实际是 |
| --- | --- | --- |
| `guge/sources/archaeology_report` | 「考古报告（书影）」 | 报告里的**刀、矛实测图版**（图一二〇，附比例尺与器物编号） |
| `guge/sources/artifact_evidence` | alt「武器的形制与结构示意」 | 模糊的道具缩略图沿金色连线串成的**溯源流程图**（模糊是有意的设计，不是坏图） |
| `guge/guide/meido_portrait` | alt「手持酥油灯，闭眼祈福」 | 闭眼侧脸的**双重曝光**，叠在红殿壁画纹样上 —— 画面里没有灯 |
| `guge/field/site_hero_01` | 「洞窟与山顶王城构成的空间关系」 | 一张**札达土林全景**，画面里既没有洞窟、也没有山顶王城 |

**做了什么** — 只改文案，一张图都没动。`archaeology_report` 顺手改得更有信息量：「1991 年《古格故城》考古报告中的器物图版：铁刀、铁刀身与铁矛头」—— 它本来就在证明「每一件道具都能追到出处」，说清是哪一版图比说「书影」有用得多。`meido_portrait` 的 alt 在案例页与首页切片各有一处，一并改掉。

**验证** — 这四条**只能靠打开图对着图注看**才发现，几何审核、构建、引用审核全是绿的（理由见 lesson「几何全绿 ≠ 内容对」）。核对方式：逐张 `read_image` + 与 `projects.ts` 的 caption/alt 并排比对。`npm run check` + `next build` 通过。核查报告留在 `_qa-output/image-content-check/`。

**遗留** — 同批还报了李花花与观潮各 2 处「存疑」：截图是滚动位置快照，alt 描述的是整块区域，图注本身没说错，不改。
