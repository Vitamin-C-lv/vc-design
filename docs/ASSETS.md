...
> **证书必须打码后公开**（2026-09-16 决定）。保留比赛的官方名称、作品名称、参赛组别、奖项等级、官方印章与版式；**抹掉**作者姓名、指导教师姓名、参赛院校/单位、证书编号、二维码/条码及其它无必要的个人字段。站点只引用打码后的衍生图，未打码的原始证书留在**仓库外**私有目录。
>
> 打码**按墨迹定位，不按固定坐标**——目测画框会整行漏掉，而漏一个名字就等于没打码。做法：在整行文字区间做横向投影得到行带（同一行的换行续行合并成一条），再在行带内做列投影，找出「标签簇 / 零墨迹间隙 / 值簇」，填充起点取间隙中点，**只擦值、保留标签**；若找不到 ≥ 4px 的零墨迹间隙（`标签：值` 贴得太近），就整行填掉——宁可整行空白，也绝不允许残留笔画碎片。填充用该行局部背景中位色、**不透明**，不用模糊（模糊有被还原的可能）。
>
> 验收判据（须同时满足）：① 填充起点左侧的标签区在 `|Δ| > 12` 阈值下**零改动**（用 `|Δ| > 2` 会把整图 JPEG 重编码噪声误判成改动）；② 擦除区内不存在面积 > 20px 的连通墨迹块，也不存在 > 4px 的碎片；③ 图像尺寸与文件集合不变。
>
> **不改写 Git 历史**：旧提交里仍是未打码版本，这是有意识的取舍——用户明确要求不要 force push、不要 rewrite history。
